...
@pytest.mark.unit
class TestStagingModelIdempotentWrite:
    """Model files are not rewritten when content is unchanged (1.0.8-N)."""
    ...
    def test_model_not_rewritten_when_content_unchanged(self, tmp_path: Path) -> None:
        gen = _make_generator_mocked(tmp_path)
        source = self._setup_source_mocks(gen)
        gen.generate_staging_model = MagicMock(
            return_value="-- Auto-generated by Dango on 2026-01-01T00:00:00\nSELECT 1"
        )

        model_file = gen.staging_dir / f"stg_{source.name}__orders.sql"
        original_content = "-- Auto-generated by Dango on 2025-01-01T00:00:00\nSELECT 1"
        model_file.write_text(original_content)

        gen.generate_all_models(sources=[source], skip_customized=True, generate_schema_yml=False)

        # Body unchanged -> file must NOT be rewritten, old timestamp stays
        assert model_file.read_text() == original_content

    def test_model_rewritten_when_content_changed(self, tmp_path: Path) -> None:
        gen = _make_generator_mocked(tmp_path)
        source = self._setup_source_mocks(gen)
        new_content = "-- Auto-generated by Dango on 2026-01-01T00:00:00\nSELECT 1, 2"
        gen.generate_staging_model = MagicMock(return_value=new_content)

        model_file = gen.staging_dir / f"stg_{source.name}__orders.sql"
        model_file.write_text("-- Auto-generated by Dango on 2025-01-01T00:00:00\nSELECT 1")

        gen.generate_all_models(sources=[source], skip_customized=True, generate_schema_yml=False)

        assert model_file.read_text() == new_content

    def test_model_created_on_first_run(self, tmp_path: Path) -> None:
        gen = _make_generator_mocked(tmp_path)
        source = self._setup_source_mocks(gen)
        content = "-- Auto-generated by Dango on 2026-01-01T00:00:00\nSELECT 1"
        gen.generate_staging_model = MagicMock(return_value=content)
...
@pytest.mark.unit
class TestStripGeneratedTimestamp:
    def test_strips_marker_line(self) -> None:
        from dango.transformation.generator import _strip_generated_timestamp

        content = "-- Auto-generated by Dango on 2026-01-01T00:00:00\nSELECT 1"
        assert _strip_generated_timestamp(content) == "SELECT 1"

    def test_passthrough_when_no_marker(self) -> None:
        from dango.transformation.generator import _strip_generated_timestamp

        content = "SELECT 1  -- hand-written, no marker"
        assert _strip_generated_timestamp(content) == content
