...
def _lock_error(
    ...
    """Build the exception pipeline.load() actually raises for a DuckDB lock conflict.

    dlt's Pipeline.load() always wraps step exceptions in PipelineStepFailed —
    the underlying DestinationConnectionError never propagates directly.
    """
    inner = DestinationConnectionError(
        client_type="duckdb", dataset_name="main", reason=reason, inner_exc=Exception(reason)
    )
    fake_pipeline = MagicMock(pipeline_name="test", has_pending_data=False)
    return PipelineStepFailed(fake_pipeline, "load", "test_load_id", inner, None)
...
class TestLoadWithLockRetry:
    ...
    def test_raises_after_max_retries(self, tmp_path):
        """Raises PipelineStepFailed after all retries exhausted."""
        runner = _make_runner(tmp_path)
        pipeline = MagicMock()
        ...
        with (
            ...
            with pytest.raises(PipelineStepFailed, match="Could not set lock"):
    ...
    def test_non_lock_error_not_retried(self, tmp_path):
        ...
        with (
            ...
            with pytest.raises(PipelineStepFailed, match="Wrong credentials"):
                runner._load_with_lock(pipeline, "test_source")
