...
def oauth(ctx: click.Context) -> None:
    ...
    Commands:
      dango oauth google_sheets      Authenticate with Google Sheets
      dango oauth google_analytics   Authenticate with Google Analytics (GA4)
      dango oauth google_ads         Authenticate with Google Ads
      dango oauth facebook_ads       Authenticate with Facebook Ads
    """
    pass
...
def oauth_list(ctx: click.Context) -> None:
    ...
    try:
        ...
        if not credentials:
            ...
            console.print("  dango oauth google_sheets")
        ...
        console.print("\n")
        console.print(table)
        console.print("\n[dim]To re-authenticate: dango oauth <source_type>[/dim]")
    ...
    try:
        ...
        if expiring_soon:
            ...
            for cred in expiring_soon:
                ...
                console.print(
                    f"    [cyan]Re-authenticate: dango oauth refresh {cred.source_type}[/cyan]\n"
                )
...
def oauth_check(ctx: click.Context) -> None:
    ...
    try:
        ...
        if not credentials:
            ...
            console.print("  [dim]→ Run: dango oauth <provider> to authenticate[/dim]")
        else:
            for cred in credentials:
                if cred.is_expired():
                    status = "[red]EXPIRED[/red]"
                    action = f"[dim]→ Run: dango oauth refresh {cred.source_type}[/dim]"
                elif cred.is_expiring_soon():
                    days_left = cred.days_until_expiry()
                    status = f"[yellow]Expires in {days_left}d[/yellow]"
                    action = (
                        f"[dim]→ Consider refreshing: dango oauth refresh {cred.source_type}[/dim]"
                    )
                else:
                    status = "[green]Active[/green]"
                    action = ""
        ...
        if all_configured and credentials:
            ...
            console.print("  [dim]Authenticate with: dango oauth <provider>[/dim]")
        else:
...
def oauth_setup(ctx: click.Context, provider: str) -> None:
    ...
    try:
        ...
        if not Confirm.ask("Ready to enter credentials?", default=True):
            ...
            console.print(f"[dim]Run again when ready: dango oauth setup {provider.lower()}[/dim]")
            return
