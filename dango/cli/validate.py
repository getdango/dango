...
class ProjectValidator:
    ...
    def _check_project_structure(self):
        ...
        for dir_path in required_dirs:
            ...
            if full_path.exists():
                self.results.append(ValidationResult(f"Directory: {dir_path}", "pass"))
            else:
                self.results.append(
                    ValidationResult(
                        f"Directory: {dir_path}", "warn", f"Missing directory: {dir_path}"
                    )
                )
    ...
    def _check_config_files(self):
        ...
        if project_yml.exists():
            try:
                ...
                self.results.append(
                    ValidationResult(
                        "Config: project.yml", "pass", f"Project: {config.project.name}"
                    )
                )
            except Exception as e:
                self.results.append(
                    ValidationResult("Config: project.yml", "fail", f"Invalid YAML: {str(e)[:100]}")
                )
        else:
            self.results.append(
                ValidationResult(
                    "Config: project.yml", "fail", "File not found. Run 'dango init' first."
                )
            )
        ...
        if sources_yml.exists():
            try:
                ...
                self.results.append(
                    ValidationResult(
                        "Config: sources.yml", "pass", f"{len(sources)} source(s) configured"
                    )
                )
            except Exception as e:
                self.results.append(
                    ValidationResult("Config: sources.yml", "fail", f"Invalid YAML: {str(e)[:100]}")
                )
        else:
            self.results.append(
                ValidationResult(
    ...
    def _check_data_sources(self):
        ...
        try:
            ...
            if not sources:
                self.results.append(
                    ValidationResult("Data Sources", "warn", "No enabled sources found")
                )
                return
            ...
            for source in sources:
                ...
                self.results.append(
                    ValidationResult(f"Source: {source.name}", "pass", f"Type: {source.type.value}")
                )

        except Exception as e:
            self.results.append(
                ValidationResult("Data Sources", "fail", f"Error checking sources: {str(e)[:100]}")
            )
    ...
    def _check_custom_sources(self):
        ...
        try:
            ...
            unreferenced = check_unreferenced_custom_sources(self.project_root, config.sources)
            ...
            self.results.append(
                ValidationResult(
                    "Custom Sources", "fail", f"Error checking custom sources: {str(e)[:100]}"
                )
            )
            ...
            for source in oauth_sources:
                ...
                if oauth_cred.is_expired():
                    ...
                    if oauth_cred.expires_at:
                        ...
                        self.results.append(
                            ValidationResult(f"OAuth: {source_name}", "pass", "Valid (no expiry)")
                        )
    ...
    def _check_dbt_setup(self):
        ...
        if dbt_project_yml.exists():
            self.results.append(ValidationResult("dbt: dbt_project.yml", "pass"))
        else:
            self.results.append(ValidationResult("dbt: dbt_project.yml", "fail", "File not found"))

        # Check profiles.yml
        profiles_yml = self.project_root / "dbt" / "profiles.yml"
        if profiles_yml.exists():
            self.results.append(ValidationResult("dbt: profiles.yml", "pass"))
        else:
            self.results.append(ValidationResult("dbt: profiles.yml", "fail", "File not found"))
        ...
        if staging_dir.exists():
            ...
            if staging_models:
                self.results.append(
                    ValidationResult(
                        "dbt: Staging models", "pass", f"{len(staging_models)} model(s) found"
                    )
                )
            else:
                self.results.append(
    ...
    def _check_database(self):
        ...
        if db_path.exists():
            try:
                ...
                self.results.append(
                    ValidationResult(
                        "Database: DuckDB", "fail", f"Cannot open database: {str(e)[:100]}"
                    )
                )
        else:
            self.results.append(
                ValidationResult(
                    "Database: DuckDB", "warn", "Database not found. Run 'dango sync' to create."
                )
            )
    ...
    def _check_dependencies(self):
        ...
        for module_name, display_name in required_packages:
            try:
                ...
                if spec is not None:
                    self.results.append(ValidationResult(f"Package: {display_name}", "pass"))
                else:
                    self.results.append(
                        ValidationResult(
                ...
                self.results.append(
                    ValidationResult(
                        f"Package: {display_name}", "fail", f"Package not installed: {display_name}"
                    )
                )
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self.results.append(ValidationResult("CLI: dbt", "pass", version_line))
            else:
                self.results.append(ValidationResult("CLI: dbt", "fail", "dbt command failed"))
        except FileNotFoundError:
            self.results.append(ValidationResult("CLI: dbt", "fail", "dbt not found in PATH"))
        except Exception as e:
            self.results.append(
                ValidationResult("CLI: dbt", "fail", f"Error checking dbt: {str(e)[:100]}")
            )
        ...
        for dir_path in check_dirs:
            ...
            if full_path.exists():
                ...
                try:
                    test_file.touch()
                    test_file.unlink()
                    self.results.append(ValidationResult(f"Permissions: {dir_path}", "pass"))
                except Exception:
                    self.results.append(
                        ValidationResult(
    ...
    def _display_results(self):
        ...
        for category, results in categories.items():
            ...
            for result in results:
                ...
                table.add_row(clean_name, status_str, result.message)
