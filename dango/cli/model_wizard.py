        ...
        try:
            ...
            if model_layer == "marts":
                ...
                next_steps = (
                    ...
                    f"1. Edit: {model_path.relative_to(self.project_root)}\n"
                    f"2. Run:  [cyan]dango run --select {model_name.replace('.sql', '')}[/cyan]\n"
                    f"3. View: [cyan]{metabase_url}[/cyan]\n\n"
                    f"[dim]Remove: dango model remove {model_name.replace('.sql', '')}[/dim]"
                )
            else:
                next_steps = (
                    f"[dim]Next Steps:[/dim]\n\n"
                    f"1. Edit: {model_path.relative_to(self.project_root)}\n"
                    f"2. Run:  [cyan]dango run --select {model_name.replace('.sql', '')}[/cyan]\n"
                    f"3. Use in marts: {{{{ ref('{model_name.replace('.sql', '')}') }}}}\n\n"
                    f"[dim]Remove: dango model remove {model_name.replace('.sql', '')}[/dim]"
                )
