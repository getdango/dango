...
class ProjectInitializer:
    ...
    def initialize(self, skip_wizard: bool = False, force: bool = False):
        ...
        try:
            ...
            # Create CLAUDE.md — LLM entry point for the project
            self._create_claude_md(config)
...
"""
    ...
    def _create_claude_md(self, config: DangoConfig) -> None:
        """Create CLAUDE.md — LLM entry point for the project."""
...
{config.project.purpose or "*(Not defined — edit .dango/project.yml)*"}
...
|-------|--------|---------|
| Staging | `staging.*` | ✅ Clean, analysis-ready — start here |
...
dango start                   # Start the full platform (Metabase + UI)
dango doctor                  # Check credential health
dango mcp setup               # Connect Claude Code / Cursor / Windsurf
```
...
.dango/
  project.yml     # Project metadata
  sources.yml     # Data source definitions
  schedules.yml   # Sync schedules
dbt/
  models/
    staging/      # stg_<source>__<entity>.sql
    intermediate/ # int_<description>.sql
    marts/        # fct_<metric>.sql, dim_<entity>.sql
data/
  warehouse.duckdb  # DuckDB warehouse (do not edit directly)
```

---
*Generated by Dango. Edit freely — this file is yours.*
"""
