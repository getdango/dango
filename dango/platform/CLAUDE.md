...
| `cloud/backup.py` | SSH-based backup and rollback | `create_backup`, `rollback`, `list_local_backups`, `rotate_local_backups`, `BackupManifest`, `BackupResult`, `RestoreResult`, `BACKUP_FILES`, `BACKUP_DIRS`, `SECRET_FILES`, `get_remote_compose_project_name()` |
...
- **`render_docker_compose()` (1.0.8-AJ)** — renders `docker-compose.yml` from a project's current `DangoConfig` (`project_name`/`project_id`/`metabase_port`/`dbt_docs_port`); a pure function of config with no free-form user-customizable sections, safe to call unconditionally and repeatedly (idempotent — unchanged config produces a byte-for-byte identical file). Called once by `cli/init.py._create_docker_compose()` at `dango init`, and again by `common/startup.py`'s `start_docker_services()` before every `dango start` — the file used to be rendered only at init and never updated, so a `metabase_port`/`dbt_docs_port` change in `project.yml` (including via this module's own port-conflict error messages, which recommend exactly that edit) silently had no effect. Lives here rather than in `cli/init.py` (where it originated) because `cli/` is Level 3 and `platform/` is Level 2 — a Level 2 caller can't import Level 3.
- **Persisted project identity (1.0.8-Q9)** — `get_compose_project_name()` reads a stable `project.id` (UUID) persisted in project.yml at `dango init` time, instead of hashing the project path (the old scheme was provably unstable — path casing/symlink resolution could change the hash for the identical directory, and a moved project directory had no way to reconnect to its pre-move containers). A pre-upgrade project without `project.id` is migrated lazily: `DockerManager._resolve_or_migrate_project_id()`, called at the very start of both `start_services()` and `stop_services()` (before `compose_project_name` is read for any other purpose), checks real Docker state via Q8's `_get_existing_container_working_dirs()` for the legacy path-hash name and either adopts that hash as `project.id` (containers already exist there — zero disruption) or generates a fresh UUID (nothing exists), persisting the result back to project.yml via `ConfigLoader.save_project_context()`. Never re-runs once `project.id` is persisted. `get_compose_project_name()` itself falls back to the deterministic legacy hash (never a random value) for a project not yet migrated, so read-only callers invoked before the first post-upgrade start/stop stay consistent with what migration will adopt. `cloud/backup.py`'s SSH-based `stop_services()`/`start_services()` (and the CLI's `deploy_provision.py`/`remote_repair.py`/`deploy.py destroy`) read the *remote* server's persisted id via `get_remote_compose_project_name()` (SSH `cat project.yml`, same legacy-hash fallback) — a local module-level constant can't know the remote id, since it's resolved on the server itself.
- **Project-identity collision guard (1.0.8-Q8)** — defense in depth on top of Q9's fix. `DockerManager._assert_no_identity_collision()` is called right after `_resolve_or_migrate_project_id()` at the start of both `start_services()` and `stop_services()`; it reads Docker's own `com.docker.compose.project.working_dir` label via `_get_existing_container_working_dirs()` and raises `DockerIdentityCollisionError` (`dango/exceptions.py`) on a confirmed mismatch against `self.project_root`. Fails open (never blocks) if the Docker check itself can't be performed — only a confirmed mismatch raises. `stop_services()` also re-verifies via the same label check after `docker compose down` reports exit 0, since exit 0 is not proof nothing is left running. `cli/commands/docker_audit.py` (`dango docker-audit`) is the machine-wide diagnostic/cleanup companion — this guard only protects the two DockerManager methods; it does not extend to the raw `docker compose stop/start` calls in `cloud/scheduled_backup.py`, which don't go through `DockerManager` at all.
