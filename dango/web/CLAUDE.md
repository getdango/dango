...
FastAPI web server providing REST API and WebSocket for managing Dango data pipelines. Serves the dashboard UI, proxies Metabase and dbt docs, and exposes endpoints for source sync, CSV upload, and platform health.
...
| `routes/ui.py` | `/`, `/health`, `/logs`, `/api`, `/api/docs`, `/api/redoc` | `_inject_version()` |
...
| File/Path | Reason |
|-----------|--------|
| `static/` | Frontend assets; changes here require frontend knowledge |
