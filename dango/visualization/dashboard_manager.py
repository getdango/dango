...
class DashboardManager:
    ...
    def get_collections(self) -> list[dict[str, Any]]:
        ...
        try:
            response = requests.get(
                f"{self.metabase_url}/api/collection", headers=self._get_headers(), timeout=10
            )
    ...
    def get_dashboards(self, collection_id: int | None = None) -> list[dict[str, Any]]:
        ...
        try:
            # Get all dashboards
            response = requests.get(
                f"{self.metabase_url}/api/dashboard", headers=self._get_headers(), timeout=10
            )
    ...
    def get_dashboard_details(self, dashboard_id: int) -> dict[str, Any] | None:
        """
    ...
    def get_cards(self, collection_id: int | None = None) -> list[dict[str, Any]]:
        """
        ...
        try:
            # Get all cards
            response = requests.get(
                f"{self.metabase_url}/api/card", headers=self._get_headers(), timeout=10
            )
    ...
    def get_card_details(self, card_id: int) -> dict[str, Any] | None:
        """
        ...
        try:
            response = requests.get(
                f"{self.metabase_url}/api/card/{card_id}", headers=self._get_headers(), timeout=10
            )
    ...
    def get_timelines(self, collection_id: int | None = None) -> list[dict[str, Any]]:
        """
        ...
        try:
            response = requests.get(
                f"{self.metabase_url}/api/timeline", headers=self._get_headers(), timeout=10
            )

            if response.status_code != 200:
                console.print(
                    f"[yellow]Warning: Could not get timelines: {response.status_code}[/yellow]"
                )
                return []
    ...
    def _create_card_from_yaml(
        self, card_data: dict[str, Any], collection_id: int | None = None
    ) -> int | None:
        """
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                console.print(
                    f"[yellow]Warning: Failed to create card '{card_data.get('name')}': {response.status_code}[/yellow]"
                )
                return None

        except Exception as e:
            console.print(
                f"[yellow]Warning: Error creating card '{card_data.get('name')}': {e}[/yellow]"
            )
            return None
    ...
    def _create_dashboard_from_yaml(
        ...
        try:
                ...
                if overwrite:
                    ...
                    try:
                        ...
                        if delete_response.status_code not in [200, 204]:
                            console.print(
                                f"[yellow]Warning: Failed to delete existing dashboard '{dashboard_name}'[/yellow]"
                            )
                            return None
                    except Exception as e:
                        console.print(
                            f"[yellow]Warning: Error deleting dashboard '{dashboard_name}': {e}[/yellow]"
                        )
                        return None
                else:
            ...
            if response.status_code not in [200, 201]:
                console.print(
                    f"[yellow]Warning: Failed to create dashboard '{dashboard_name}': {response.status_code}[/yellow]"
                )
                return None
            ...
            console.print(
                f"[yellow]Warning: Error creating dashboard '{dashboard_data.get('name')}': {e}[/yellow]"
            )
            return None
    ...
    def _create_question_from_yaml(
        ...
        try:
            ...
            if question_name in existing_questions:
                if overwrite:
                    ...
                    try:
                        ...
                        if delete_response.status_code not in [200, 204]:
                            console.print(
                                f"[yellow]Warning: Failed to delete existing question '{question_name}'[/yellow]"
                            )
                            return None
                    except Exception as e:
                        console.print(
                            f"[yellow]Warning: Error deleting question '{question_name}': {e}[/yellow]"
                        )
                        return None
                else:
            ...
            console.print(
                f"[yellow]Warning: Error creating question '{question_data.get('name')}': {e}[/yellow]"
            )
            return None

    def save_to_files(
        self, include_personal: bool = False, collections: list[str] | None = None
    ) -> dict[str, Any]:
        """
        ...
        summary["success"] = (
            len(summary["exported_dashboards"]) > 0
            or len(summary["exported_questions"]) > 0
            or len(summary["exported_models"]) > 0
            or len(summary["exported_metrics"]) > 0
            or len(summary["exported_timelines"]) > 0
        )
        return summary

    def load_from_files(self, overwrite: bool = False, dry_run: bool = False) -> dict[str, Any]:
        """
        ...
        try:
                        ...
                        if dashboard_name in existing_dashboards:
                            if not overwrite:
                                ...
                                summary["would_overwrite"].append(
                                    {"type": "dashboard", "name": dashboard_name}
                                )
                        ...
                        try:
                            ...
                            if card_name in existing_cards:
                                if not overwrite:
                                    ...
                                    summary["would_overwrite"].append(
                                        {"type": card_type_name, "name": card_name}
                                    )
                            ...
                            if dry_run:
                                ...
                                if card_id:
                                    created_items.append(
                                        {"type": card_type_name, "id": card_id, "name": card_name}
                                    )
                                    summary[summary_key].append(
                                        {
                                            "name": card_name,
                                    ...
                                    raise Exception(
                                        f"Failed to import {card_type_name}: {card_name}"
                                    )
            ...
            summary["success"] = (
                len(summary["imported_dashboards"]) > 0
                or len(summary["imported_questions"]) > 0
                or len(summary["imported_models"]) > 0
                or len(summary["imported_metrics"]) > 0
                or len(summary["skipped"]) > 0
            )
            ...
            if not dry_run and (created_items or backups):
                ...
                for item in created_items:
                    try:
                        ...
                        console.print(
                            f"  [yellow]Warning: Could not delete {item['name']}: {rollback_error}[/yellow]"
                        )
...
def import_dashboards(project_root: Path, overwrite: bool = False) -> dict[str, Any]:
    ...
    try:
        ...
        return {"success": False, "imported": [], "skipped": [], "errors": [str(e)]}
